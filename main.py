"""
AffanMarvel Auto-Poster — GROQ VERSION
========================================
Flow 2 : Google News RSS  — 10 articles per topic (5 topics = 50 raw)
Flow 3 : Web Scraping     — 10 articles per site  (2 sites  = 20 raw)
After  : Deduplicate → Groq AI rewrite → WordPress DRAFT
Groq is free with 14,400 requests/day — no rate limit problems!
"""
...
Write a NEWS BLOG POST based on this info:

Original Title : {title}
Category       : {category}
Summary        : {summary if summary else "Write based on the title only."}

STRICT RULES:
1. Write a NEW catchy title — do NOT copy the original title.
2. Write EXACTLY 3 paragraphs:
   - Paragraph 1: The main news — what happened.
   - Paragraph 2: Why it matters — fan reaction or background context.
   - Paragraph 3: What comes next — end with one question for readers.
3. Total word count: 200 to 280 words only. No padding.
4. Do NOT mention the source website ({source}).
5. Wrap each paragraph in <p> HTML tags.

Return ONLY a valid JSON object. No markdown. No extra text. No backticks:
{{"title": "Your new title here", "content": "<p>Para 1.</p><p>Para 2.</p><p>Para 3.</p>", "excerpt": "One sentence summary max 25 words.", "tags": ["tag1", "tag2", "tag3", "tag4", "tag5"]}}"""
...
# ─────────────────────────────────────────────────────────────────────────────
# 9. WORDPRESS — GET CATEGORY ID
# ─────────────────────────────────────────────────────────────────────────────

_cat_cache = {}

def get_wp_category_id(category_name):
    if category_name in _cat_cache:
        return _cat_cache[category_name]
    try:
        resp = requests.get(
        ...
        if isinstance(data, list) and data:
            for cat in data:
                if cat.get("name", "").lower() == category_name.lower():
                    _cat_cache[category_name] = cat["id"]
                    return cat["id"]
            _cat_cache[category_name] = data[0]["id"]
            return data[0]["id"]
    except Exception as e:
        print(f"  ⚠ Category lookup error: {e}")
    _cat_cache[category_name] = 1
    return 1

# ─────────────────────────────────────────────────────────────────────────────
# 10. WORDPRESS — GET OR CREATE TAGS
# ─────────────────────────────────────────────────────────────────────────────
...
def main():
    print("\n" + "=" * 60)
    print(f"  AffanMarvel Auto-Poster (Groq) — {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
    print("=" * 60)
